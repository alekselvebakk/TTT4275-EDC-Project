...
# Appends a column vector of ones to the right side of given matrix
def append_unit_endings(x_vector):
    rows = x_vector.shape[0]
    return np.hstack((x_vector, np.ones((rows,1))))
...
def check_match(predicted, real):
    return np.argmax(real) == np.argmax(predicted)
...
if __name__ == "__main__":
    ...
    # a) Using first 30 samples as training and last 20 as testing
